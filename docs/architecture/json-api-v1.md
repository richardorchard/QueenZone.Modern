...
| `/api/v1/content/*` | Public, read-only archive content for the mobile app: news, biography, discography, timeline, Freddie Tribute (#726), and photo galleries (#743). No authentication required. Photo image URLs are `cdn.queenzone.org` via `PhotoImageUrl` (not App Service). Category items default and clamp `pageSize` to `PhotoRoutes.CategoryPageSize` (24) so pages match `/photography/{slug}`. |
