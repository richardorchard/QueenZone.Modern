...
| `/api/v1/auth/*` | Mobile OAuth2 PKCE + tokens (see issues #720 / #721). `POST /api/v1/auth/token` also accepts `grant_type=password` for operator-created accounts (#1351; same access/refresh pair as authorization-code exchange). Public `GET /api/v1/auth/providers` lists configured Google/Microsoft/Discord/GitHub/Apple buttons matching `/account/login`. |
| `/api/v1/admin` | Admin status probe; future admin JSON must use the same `Admin` policy (#723) |
| `/api/v1/content/*` | Public, read-only archive content for the mobile app: news, long-form editorial articles (#1186), biography, discography, timeline (`GET /api/v1/content/timeline/{id}` returns the same list-card event for the mobile On This Day destination; missing or unpublished ids are Problem Details 404; #1288), Freddie Tribute (#726), photo galleries (#743), fan-performance listings (#747 / #748 / #750), the current Home poll (#1089), and a random published trivia fact (#1100). No authentication required for those reads. `GET /api/v1/content/articles` pages published `IArticlesRepository` / `ArticleItem` rows via `GetArchivePageAsync` + `GetPublishedCountAsync` (default `pageSize` 20, `ArticlesRoutes.ArchivePageSize`). Items are `{ id, title, excerpt, publishedAt, detailPath, categoryName? }` — not news, no images, no forum topic. `GET /api/v1/content/articles/{id}` adds sanitized `body` (`ArticleContent.FormatBody`) and string `source`; missing or unpublished ids are Problem Details 404. Community submissions (`IArticleRepository`, slug ids) stay off this surface. `GET /api/v1/content/trivia/random` returns `{ id, text }` plus optional `category` / `difficulty` / `source`, or JSON `null` when none is published (same empty-set contract as `/quotes/random`). `GET /api/v1/content/home-poll` returns JSON `null` when none is live (same as `/quotes/random`). Counts and percentages are public; an optional Bearer token only marks `viewerHasVoted` / `selectedOptionId`. `POST /api/v1/content/home-poll/votes` `{ optionId }` requires `MobileMemberPolicy` and casts one final ballot via `HomePollVoteService` (same write path as the website Index POST). `GET /api/v1/content/news` accepts optional `decade` (e.g. `2010`) to filter server-side to that 10-year span (floored to the decade start; out-of-range years are ignored). News list and detail items include additive nullable `imageUrl` / `thumbnailUrl` (resolved from an articles blob key via `/ugc/articles/{blobName}` and `?size=thumb`; both are `null` when the article has no image or the reference is a gallery pick). News list items also include additive nullable `topicId` / `replyCount` (batched counts only; no bodies). Detail adds `topicId`, `discussionReplyCount`, and `discussionPreview` (last N forum replies, not the opening post; N = 2, or 1 if only one reply). All discussion fields are `null` when the article has no linked topic so clients omit the block. Photo image URLs are `cdn.queenzone.org` via `PhotoImageUrl` (not App Service). Category list/detail/items reuse `PublicQueryCacheService` photo helpers (same as Razor). Category items default and clamp `pageSize` to `PhotoRoutes.CategoryPageSize` (24) so pages match `/photography/{slug}`. Photo items include `detailPath` and `categoryPath`. Fan-performance items include `durationSeconds` (MPEG metadata when the songfile is readable) and `audioPath`. Streaming `GET /api/v1/content/fan-performances/{id}/audio` requires `MobileMemberPolicy` and reuses `FanPerformanceEndpoints.ServeAudioAsync` (private `songfiles` blob, HTTP range processing). The website cookie path `/fan-performances/{id}/audio` is unchanged. |
| `/api/v1/search` | Public whole-site search over the shared `SearchDocument` index (`ISiteSearchService` / `dbo.SearchDocument_Search`). Same visibility as website `/search`. Query `q` plus optional `type` (`SiteSearchContentType.Normalize`; unknown values search all types). Empty or whitespace `q` returns `200` with an empty page (not `400`). Default `pageSize` is 20 (`SearchModel.PageSize`); max 100. `totalCount` is capped at `SiteSearchLimits.MaxRankedMatches` (1000). Untyped All applies that cap as `FREETEXTTABLE` `top_n_by_rank` so common terms stay inside the SQL command timeout. Typed search (`type=discography`, and the other `SiteSearchContentType` values) filters `ContentType` first, then applies the same cap, so a track-title body hit is not crowded out by news/forum. A SQL command timeout (`Execution Timeout Expired` / `-2`) is Problem Details `504` (not an empty `200` and not `500`). Items include `sourceKey` and an optional numeric `id` parsed from that key. Summaries are plain text (no `<mark>` HTML). Rate limit: `QueenZoneRateLimitPolicies.Search`. |
