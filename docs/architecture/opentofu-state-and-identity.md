...
The same plan identity now also has Reader and the config-reader custom role on
`Queenzone-Dev-RG`; the apply identity has Contributor on that group. The custom
role's assignable scopes contain both QueenZone groups, with its single action
unchanged. No subscription-wide role assignment was added. Existing state
container permissions cover both `production.tfstate` and `dev.tfstate`.
...
| --- | --- | --- |
| Plan | Workers Scripts Read | Zone Read, DNS Read, Zone Settings Read, SSL and Certificates Read, Workers Routes Read, Zone WAF Read |
| Apply | Workers Scripts Edit | Zone Read, DNS Edit, Zone Settings Edit, SSL and Certificates Edit, Workers Routes Edit, Zone WAF Edit |
...
The zone WAF permissions are required for the managed `cloudflare_ruleset`
resources. The plan token first required `Zone WAF Read` after the crawler and
archive-cache rulesets were applied on 2026-09-09; without it, provider refresh
fails with `403 Forbidden` before an otherwise safe plan can complete.
