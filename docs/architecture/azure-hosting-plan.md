...
`WEBSITE_WARMUP_PATH` points at `/health`, **not** `/warmup`, deliberately. `WEBSITE_WARMUP_STATUSES` stays unset, so any HTTP response proves the container is listening; requiring exactly 200 turned an internal-host 400 into a 230-second crash loop (#684). QueenZone short-circuits probe paths before its explicit host filter, so `/health` now returns 200 even when App Service uses a link-local Host header. `deploy.yml`'s own "Warm up custom domain" step remains the strict readiness gate: it polls `/warmup` and requires `/` to serve the new `data-build-version`, then post-deploy smoke checks representative public routes.
