...
**Phase 1 (this change):** `.github/workflows/mobile-device-smoke.yml` runs on `workflow_dispatch` and weekdays at 04:00 UTC. Android and iOS jobs are equal; both must pass for that workflow run to be green. Maestro app flows are **not** retried. An iOS driver-startup failure may retry once only when no JUnit file exists, proving no flow began; the harness reboots the Simulator and preserves the first driver log separately. Failures upload `src/QueenZone.Mobile/maestro-results/` (screenshots, JUnit, runner/app logs). A forced assertion is `workflow_dispatch` with `prove_failure=true` (runs `maestro/prove-failure.yaml`).

**On-demand journeys (#1071):** `maestro/journeys.yaml` is three extra flows (forum attach + open, news story → discussion, masthead unread). `smoke.yaml` stays the 9 #872 flows. Journeys run as `mobile-android-journeys` / `mobile-ios-journeys` on `workflow_dispatch` (`suite=journeys` or `both`) and weekdays at 16:00 UTC. They do not run at 04:00 and are not a PR gate. Both platforms must pass. App flows are not retried; the same pre-flow iOS driver-startup exception applies. Failure uploads the same `maestro-results/` tree.
