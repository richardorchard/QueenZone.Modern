...
| `mobile-android-smoke` / `mobile-ios-smoke` | Maestro device smoke against a Release-embedded build baked at the local Testing contract host (#872 Option A / #1322) | **No (Phase 1).** `.github/workflows/mobile-device-smoke.yml` is `workflow_dispatch` + weekday 04:00 UTC only. Do not add these names to branch protection yet. |
...
**Phase 1 (this change):** `.github/workflows/mobile-device-smoke.yml` runs on `workflow_dispatch` and weekdays at 04:00 UTC. Android and iOS jobs are equal; both must pass for that workflow run to be green. The scheduled smoke and journey jobs retain their GitHub-hosted runners. Maestro app assertions are **not** retried. One Android device-server or emulator transport loss (`DeviceServerDied`, hierarchy `DEADLINE_EXCEEDED`, or `emulator-5554` gone) may recover ADB and retry the same flows when the emulator is still alive; if the emulator process is dead, the hosted Android job boots a new AVD (`test-retry`, `force-avd-creation: true`) and reruns once. Hosted smoke and journeys emulators use API 34, emulator channel `stable`, 3 cores / 4096M RAM / 512M heap and `-no-snapshot` on both attempts (#1458). Do not add a third identical attempt. Transport-death artifacts include streaming `logcat.txt` plus `emulator-host.txt` (adb/qemu/dmesg/AVD config) because a post-death `adb logcat -d` is empty once the device is gone (#1454). Failure logs name transport death versus a selector miss. An iOS driver-startup failure may likewise retry once only when no JUnit file exists; the harness reboots the Simulator and preserves the first driver log separately. The authenticated smoke flow may repeat only the iOS smoke-auth deep link when the profile explicitly remains signed out after accepting the system Open prompt; it does not repeat a failed app assertion. Failures upload `src/QueenZone.Mobile/maestro-results/` (screenshots, JUnit, runner/app logs). A forced assertion is `workflow_dispatch` with `prove_failure=true` (runs `maestro/prove-failure.yaml`).
