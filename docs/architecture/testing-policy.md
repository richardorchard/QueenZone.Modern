...
| `mobile-android-journeys` / `mobile-ios-journeys` (`suite=release`) | P0 release Maestro suite (private-message lifecycle, forum topic/reply, news suggestion tracking; #1411) | **No.** Manual release acceptance on both platforms. It uses the deterministic Testing host and is not scheduled or required for PRs. |
...
**Phase 1 (this change):** `.github/workflows/mobile-device-smoke.yml` runs on `workflow_dispatch` and weekdays at 04:00 UTC. Android and iOS jobs are equal; both must pass for that workflow run to be green. Hosted Android pins stable Emulator 36.5.11 rather than accepting unreviewed emulator updates and uses the supported direct `-gpu swiftshader` backend; the former `swiftshader_indirect` mode is deprecated, while the generic `software` selector chose Lavapipe/Swangle and exited when Maestro started its device driver. Maestro app assertions are **not** retried. One Android ADB transport loss may retry only when Maestro's debug log records a dead or offline device; the harness recovers ADB, reinstalls the same APK, and preserves the first diagnostics. An iOS driver-startup failure may likewise retry once only when no JUnit file exists; the harness reboots the Simulator and preserves the first driver log separately. The authenticated smoke flow may repeat only the iOS smoke-auth deep link when the profile explicitly remains signed out after accepting the system Open prompt; it does not repeat a failed app assertion. Failures upload `src/QueenZone.Mobile/maestro-results/` (screenshots, JUnit, runner/app logs). A forced assertion is `workflow_dispatch` with `prove_failure=true` (runs `maestro/prove-failure.yaml`).
...
with `suite=release` and runs on both platforms through the extended-suite job
pair. Each flow clears app state, injects only the Testing fixture token, and
