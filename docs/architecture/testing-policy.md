...
**On-demand journeys (#1071):** `maestro/journeys.yaml` is three extra flows (forum attach + open, news story → discussion, masthead unread). `smoke.yaml` stays the 9 #872 flows. Journeys run as `mobile-android-journeys` / `mobile-ios-journeys` on `workflow_dispatch` (`suite=journeys` or `both`) and weekdays at 16:00 UTC. They do not run at 04:00 and are not a PR gate. Both platforms must pass. No retries. Failure uploads the same `maestro-results/` tree.
