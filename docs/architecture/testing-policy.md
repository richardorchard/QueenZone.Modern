...
**Phase 1 (this change):** `.github/workflows/mobile-device-smoke.yml` runs on `workflow_dispatch` and weekdays at 04:00 UTC. Android and iOS jobs are equal; both must pass for that workflow run to be green. The scheduled smoke and journey jobs retain their GitHub-hosted runners. Maestro app assertions are **not** retried. One Android ADB transport loss may retry only when Maestro's debug log records a dead or offline device; the harness recovers ADB, reinstalls the same APK, and preserves the first diagnostics. An iOS driver-startup failure may likewise retry once only when no JUnit file exists; the harness reboots the Simulator and preserves the first driver log separately. The authenticated smoke flow may repeat only the iOS smoke-auth deep link when the profile explicitly remains signed out after accepting the system Open prompt; it does not repeat a failed app assertion. Failures upload `src/QueenZone.Mobile/maestro-results/` (screenshots, JUnit, runner/app logs). A forced assertion is `workflow_dispatch` with `prove_failure=true` (runs `maestro/prove-failure.yaml`).

**On-demand journeys (#1071):** `maestro/journeys.yaml` is three extra flows (forum attach + open, news story → discussion, masthead unread). `smoke.yaml` stays the 9 #872 flows. Journeys run as `mobile-android-journeys` / `mobile-ios-journeys` on `workflow_dispatch` (`suite=journeys` or `both`) and weekdays at 16:00 UTC. They do not run at 04:00 and are not a PR gate. Both platforms must pass. App assertions are not retried; the same pre-assertion Android transport and pre-flow iOS driver-startup exceptions apply. Failure uploads the same `maestro-results/` tree.
