...
**Version 2 · 22 August 2026.** Changes since v1: navigation restructured to mirror the website
(Home · News · Photography · Archive · Forum, with a new Archive hub screen and Profile behind the
Home masthead avatar); all icons moved to the design system's Lucide set; style guide gained the
icon rule (now six rules) and the "where new sections live" rule.
...
| File | What it is |
|---|---|
| **`STYLE_GUIDE.md`** | **Read first, and re-read before every new screen or section.** The rules and recipes that keep new work consistent: the six rules, section anatomy, the six section shapes, spacing rhythm, colour-by-meaning table, copy rules, review checklist. |
...
| `screens/` | Renders of the approved design — iOS Home, Archive hub, story reader, profile; Android Home, Archive hub, forum, photography. Photography renders dark in these captures (the greyscale filter doesn't survive the capture) — **the prototype is the authority on imagery**. |
...
> Photography → Photo viewer → Archive hub → Forum → Thread → Search → Profile in that order.
> The five tabs are Home · News · Photography · Archive · Forum and are fixed — new archive
> sections become rows on the Archive hub, never a sixth tab. Content is
...
- No emoji, anywhere. One icon set: Lucide outline, 1.5 stroke — never a hand-drawn glyph.
