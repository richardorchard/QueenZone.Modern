    ...
    TimeProvider timeProvider)
{
    ...
    /// or speed alone, is too noisy and would catch genuine members. A registration email
    /// matching <see cref="AutoGeneratedFreemailPattern"/> is treated as an equally strong
    /// signal on its own (see <see cref="FlagIfLikelySpamAsync"/>), since that shape is not
    /// something a human picks for their own address.
    /// </summary>
    ...
    /// run, on a Microsoft freemail domain. Deliberately narrow (this exact shape only) to
    /// keep false positives on genuine, merely-unusual addresses at zero.
    /// </summary>
    internal static readonly Regex AutoGeneratedFreemailPattern = new(
        @"^[a-z]+_[0-9a-f]{12,20}@(outlook|hotmail|live|msn)\.com$",
        RegexOptions.IgnoreCase | RegexOptions.Compiled);
    ...
    /// registration email itself matches <see cref="AutoGeneratedFreemailPattern"/> — the
    /// email shape alone is enough, since a genuine member never registers with one. Runs
        ...
        CancellationToken cancellationToken)
    {
        ...
        var signature = hasLink && hasAutoGeneratedEmail
            ? "a link and an auto-generated-looking registration email"
            : hasLink
                ? "a link"
                : "an auto-generated-looking registration email";
        var reason =
