...
- `.cursor/agents/` and `.cursor/skills/orchestrate-epic/` are the Cursor issue-queue orchestrator (planner / implementer / verifier / reviewer) for one GitHub issue, an epic's children, or a list, including website and `QueenZone.Mobile`. Pin the skill as a Custom Mode. Sequential only: one issue, one subagent at a time; one review plus one response, then PR.
